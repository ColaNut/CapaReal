function plotXY_Eso1017_Octant( paras2dXY, dx, dy )
...
x_tumor = tumor_x_es - 1: 1 / 10: tumor_x_es + 1;
plot( x_tumor, ( 0 + tumor_hy_es / 2 ) * ones( size(x_tumor) ), 'Color', [0.5, 0.5, 0.5], 'LineWidth', 2.5);
plot( x_tumor, ( 0 - tumor_hy_es / 2 ) * ones( size(x_tumor) ), 'Color', [0.5, 0.5, 0.5], 'LineWidth', 2.5);

% plot the coil position -- horizental
x_tumor_short = tumor_x_es - 0.25: 1 / 10: tumor_x_es + 0.25;
plot( x_tumor_short, ( 0 + tumor_hy_es / 2 - 0.75 ) * ones( size(x_tumor_short) ), 'Color', [0, 0, 0], 'LineWidth', 2.5);
plot( x_tumor_short, ( 0 - tumor_hy_es / 2 + 0.75 ) * ones( size(x_tumor_short) ), 'Color', [0, 0, 0], 'LineWidth', 2.5);

% plot the coil position -- vertical
y_idx_coil = tumor_y_es - tumor_hy_es / 2 + 1: dy / 10: tumor_y_es + tumor_hy_es / 2 - 1;
...
% plot the coil position -- oblique
x_idx_oblique1 = tumor_x_es - 0.5: 1 / 100: tumor_x_es - 0.25;
x_idx_oblique2 = tumor_x_es + 0.25: 1 / 100: tumor_x_es + 0.5;
plot( x_idx_oblique1, x_idx_oblique1 + tumor_hy_es / 2 - 0.5, 'Color', [0, 0, 0], 'LineWidth', 2.5);
plot( x_idx_oblique1, - x_idx_oblique1 - tumor_hy_es / 2 + 0.5, 'Color', [0, 0, 0], 'LineWidth', 2.5);
plot( x_idx_oblique2, - x_idx_oblique2 + tumor_hy_es / 2 - 0.5, 'Color', [0, 0, 0], 'LineWidth', 2.5);
plot( x_idx_oblique2, x_idx_oblique2 - tumor_hy_es / 2 + 0.5, 'Color', [0, 0, 0], 'LineWidth', 2.5);
