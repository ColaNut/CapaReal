...
% % load('Case0504.mat');
% % M_sparseGVV = mySparse2MatlabSparse( sparseGVV, N_v, 'Col' );
...
% start from here
% imlement the following function to M sparse matrix version
% check the M_three matrix: NaN and Infty
% how to evaluate SPAI in papers.
...
% tic;
% Matlab_sparse = mySparse2MatlabSparse( sparseK, N_e, 'Row' );
% toc;
% bar_x_my_gmres = zeros(size(B_k));
% tic;
% disp('Calculation time of iLU: ')
% [ L_K, U_K ] = ilu( Matlab_sparse, struct('type', 'ilutp', 'droptol', 1e-1) );
% toc;
% tic;
