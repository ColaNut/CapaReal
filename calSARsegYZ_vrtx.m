function [ PntSARseg, TtrVol, MidPnts9Crdnt ] = calSARsegYZ_vrtx( m_B, n_B, ell_B, PhiYZ, x_max_vertex_B, y_max_vertex_B, Vertex_Crdnt_B, PntSegMed_B, sigma, rho )
    ...
    MidPnts9Crdnt = getMidPnts9CrdntYZ( MidPntsCrdnt );
    MidPhi = get27Phi( 2, n_v_B, ell_v_B, PhiYZ );

    % p1
    [ PntSARseg(1, :), TtrVol(1, :) ] = calPntSARseg( squeeze( MidPntsCrdnt(3, :, :) ), squeeze( MidPntsCrdnt(2, 5, :) ), ...
                                PntSegValue(1, :), MidPhi(3, :), MidPhi(2, 5), sigma, rho );
    % p2
    tmpMidCrdnt = p2Face( MidPntsCrdnt );
    tmpMidPhi   = p2FaceMidPhi( MidPhi );
    [ PntSARseg(2, :), TtrVol(2, :) ] = calPntSARseg( squeeze( tmpMidCrdnt ), squeeze( MidPntsCrdnt(2, 5, :) ), ...
                                PntSegValue(2, :), tmpMidPhi, MidPhi(2, 5), sigma, rho );

    % p3
    tmpMidCrdnt = p3Face( MidPntsCrdnt );
    tmpMidPhi   = p3FaceMidPhi( MidPhi );
    [ PntSARseg(3, :), TtrVol(3, :) ] = calPntSARseg( squeeze( tmpMidCrdnt ), squeeze( MidPntsCrdnt(2, 5, :) ), ...
                                PntSegValue(3, :), tmpMidPhi, MidPhi(2, 5), sigma, rho );

    % p4
    tmpMidCrdnt = p4Face( MidPntsCrdnt );
    tmpMidPhi   = p4FaceMidPhi( MidPhi );
    [ PntSARseg(4, :), TtrVol(4, :) ] = calPntSARseg( squeeze( tmpMidCrdnt ), squeeze( MidPntsCrdnt(2, 5, :) ), ...
                                PntSegValue(4, :), tmpMidPhi, MidPhi(2, 5), sigma, rho );

    % p5
    tmpMidCrdnt = p5Face( MidPntsCrdnt );
    tmpMidPhi   = p5FaceMidPhi( MidPhi );
    [ PntSARseg(5, :), TtrVol(5, :) ] = calPntSARseg( squeeze( tmpMidCrdnt ), squeeze( MidPntsCrdnt(2, 5, :) ), ...
                                PntSegValue(5, :), tmpMidPhi, MidPhi(2, 5), sigma, rho );

    % p6
    tmpMidCrdnt = p6Face( MidPntsCrdnt );
    tmpMidPhi   = p6FaceMidPhi( MidPhi );
    [ PntSARseg(6, :), TtrVol(6, :) ] = calPntSARseg( squeeze( tmpMidCrdnt ), squeeze( MidPntsCrdnt(2, 5, :) ), ...
