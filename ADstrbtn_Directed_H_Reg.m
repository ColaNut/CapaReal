...
if flag_XZ == 1
    for dirFlag = 1: 1: 3
        % temporarily set to be 8
        % the eight tetrahedra belong to its right upper point.
        H_XZ = zeros(x_idx_max_prm, z_idx_max_prm, 8, 3); 
        ...
        for idx = 1: 1: x_idx_max * z_idx_max
            ...
            if m >= 2 && m <= x_idx_max - 1 && ell >= 2 && ell <= z_idx_max - 1
                ...
                ell_v = 2 * ell - 1;
                PntsIdx = zeros( 3, 9 ); 
                PntsIdx = get27Pnts_KEV( m_v - 1, n_v, ell_v - 1, x_max_vertex, y_max_vertex, z_max_vertex );
        ...
        colormap jet;
        axis equal;
        axis( [ - 100 * w_x / 2 + 1, 100 * w_x / 2 - 1, - 100 * w_z / 2 + 1, 100 * w_z / 2 - 1 ]);
