...
% === % ========================== % === %
% === % Construction of coordinate % === %
% === % ========================== % === %
clc; clear;
digits;
disp('Cervix: EQS, test');
...
rho           = [   1,     1,   1020,           1050 ]';
epsilon_r_pre = [   1,   113,    113,            386 ]';
sigma         = [   0,  0.61,   0.61,          0.614 ]';
...
% save('0910Cervix_EQS.mat');
% return;
% === % ================= % === %
% === % put on electrodes % === %
% === % ================= % === % 
% cervix version: the potential on the RHS of endocervical canal is V_0; while that on the LHS is 0
...
[ sparseS, B_phi ] = PutOnElctrd_cervixThinBls( sparseS, B_phi, V_0, squeeze(mediumTable(:, y_mid, :)), tumor_x, tumor_y, ...
...
% BndryTable is for temperature matrix
BndryTable = zeros( x_max_vertex, y_max_vertex, z_max_vertex );
BM_bndryNum = 13;
for idx = 1: 1: x_idx_max * y_idx_max * z_idx_max
    [ m, n, ell ] = getMNL(idx, x_idx_max, y_idx_max, z_idx_max);
...
%                             dt, 4 * Q_s_Vector(CandiTet(itr)) + Q_met(MedVal), rho(MedVal), xi(MedVal), zeta(MedVal), cap(MedVal), rho_b, cap_b, alpha, T_blood, T_bolus, ...
