function [ K1_6, B_k_Pnt ] = fillK1_FW_2( vIdx1, vIdx2, vIdx3, vIdx4, Counter, v1Table, v2Table, v3Table, v4Table, v1_flag, v2_flag, v3_flag, v4_flag, ...
                    K1_6, B_k_Pnt, PntJ_xyz, MedVal, epsilon_r, mu_r, x_max_vertex, y_max_vertex, z_max_vertex, Vertex_Crdnt, varargin )
    ...
    % A self-checker
    if ~find(Counter == 1) || ~find(Counter == 2) || ~find(Counter == 3) || ~find(Counter == 4) 
        error('check the original feed in');
    end
    
    vIdxSet_permute = [ find(Counter == 1), find(Counter == 2), find(Counter == 3), find(Counter == 4) ];
    ...
    six_eIdx     = zeros(1, 6);
    six_eVal     = zeros(1, 6); % first row for K1; while the second for K2
    Bk_val = 0;
    ...
    % implement the getBk
    Bk_val = getBk( P1_Crdt, P2_Crdt, P3_Crdt, P4_Crdt, mainEdge, InnExtText, PntJ_xyz );
    six_eVal = get6E( P1_Crdt, P2_Crdt, P3_Crdt, P4_Crdt, mainEdge, InnExtText, mu_r(MedVal), epsilon_r(MedVal) );
