...
% % FullWave_Flag = 0;
% % if EQS_Flag
%     CaseName = '20180827_LungEQS_case4_nonlinear_FW';
...
for idx = 1: 1: x_idx_max * y_idx_max * z_idx_max
    ...
    if n == tumor_n && m >= 2 && m <= x_idx_max - 1 && ell >= 2 && ell <= z_idx_max - 1
        ...
        E_XZ(m, ell, :, :, :) = getSigmaE( Phi27, PntsCrdnt, squeeze( SegMed(m, n, ell, :, :) ), ones(size(sigma)), zeros(size(sigma)) );
    end
end
toc;

E_XZabs = zeros(x_idx_max, z_idx_max, 6, 8);
...
for idx = 1: 1: x_idx_max * z_idx_max
    ...
    if m >= 2 && m <= x_idx_max - 1 && ell >= 2 && ell <= z_idx_max - 1
        ...
        plotPntH( squeeze(E_XZabs(m, ell, :, :)), MidPnts9Crdnt, 'XZ' );
    end
end
toc;
hold on;
...
cb.Label.String = '$\left| \bar{E} \ \right|$ (V/m)';
