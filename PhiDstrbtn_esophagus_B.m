...
%     CrossN =int64( tumor_y_es / dy_B + w_y_B / ( 2 * dy_B ) + 1 + 1);
...
%     figure(1);
%     clf;
%     x_mesh = squeeze(Vertex_Crdnt_B( :, CrossN_v, :, 1))';
%     z_mesh = squeeze(Vertex_Crdnt_B( :, CrossN_v, :, 3))';
...
%     CrossEll = int64( (tumor_z_es - es_z) / dz_B + w_z_B / ( 2 * dz_B ) + 1 + 1);
%     CrossEll_v = 2 * CrossEll - 1;

%     for vIdx_B = 1: 1: x_max_vertex_B * y_max_vertex_B * z_max_vertex_B
%         [ m_v_B, n_v_B, ell_v_B ] = getMNL(vIdx_B, x_max_vertex_B, y_max_vertex_B, z_max_vertex_B);
%         if ell_v_B == CrossEll_v
...
%     figure(6);
%     clf;
%     x_mesh = squeeze(Vertex_Crdnt_B( :, :, CrossEll_v, 1))';
%     y_mesh = squeeze(Vertex_Crdnt_B( :, :, CrossEll_v, 2))';
...
if flag_YZ == 1
    ...
    CrossM =int64( tumor_x_es / dx_B + w_x_B / ( 2 * dx_B ) + 1 + 1);
    ...
    figure(11);
    clf;
    y_mesh = squeeze(Vertex_Crdnt_B( CrossM_v, :, :, 2))';
    z_mesh = squeeze(Vertex_Crdnt_B( CrossM_v, :, :, 3))';
