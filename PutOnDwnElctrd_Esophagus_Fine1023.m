function [ sparseS_1, B_phi ] = PutOnDwnElctrd_Esophagus_Fine1023( sparseS_1, B_phi, N_v, V_0, DwnElctrdMask, DwnElctrdPos, x_max_vertex_B, y_max_vertex_B, z_max_vertex_B, w_x_B, w_y_B, w_z_B, dx_B, dy_B, dz_B )
...
for n_v_B = y_vIdx_near: 1: y_vIdx_far
    for vIdx_B = 1: 1: x_max_vertex_B * z_max_vertex_B
        [ m_v_B, ell_v_B ] = getML(vIdx_B, x_max_vertex_B);
        if DwnElctrdMask(m_v_B, ell_v_B)
            p0 = N_v + ( ell_v_B - 1 ) * x_max_vertex_B * y_max_vertex_B + ( n_v_B - 1 ) * x_max_vertex_B + m_v_B;
