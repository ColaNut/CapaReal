...
disp('Test with three-fold deeper modified region; while left the lung part uneffected');
disp('Test with 3.5 times of conductiviy in adipose tissue');
disp('Test with 3.5 times of conductiviy in the injected region');
disp('Restricted Saline Injected Region in Adipose Tissue');
...
% rho           = [ 1,  1020,  1020,  1050, 1040 ]';
              % [ air, bolus, muscle, lung,  tumor,  bone,   fat, saline injected fat ]';
...
sigma         = [   0,  0.69,  0.69,  0.42,  0.68, 0.045,  0.0245, 0.69 * 3.5, 0.0245 * 3.5 ]';
...
for idx = 1: 1: x_idx_max * y_idx_max * z_idx_max
    ...
    if m > tumor_m - 10 && m < tumor_m + 10 && n > tumor_n - 10 && n < tumor_n + 10 ...
        && ell > tumor_ell && ell < tumor_ell + 10
        if m == 20 && n == 15 && ell == 30
            ;
        end
        ...
        for fIdx = 1: 1: 6
            for nIdx = 1: 1: 8
                ...
                if is_sub_electrode_ROI( squeeze( Tet48Crdnt(fIdx, nIdx, :) ) )
                    if SegMed(m, n, ell, fIdx, nIdx) ~= uint8(4)
