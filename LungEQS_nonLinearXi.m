...
% === === === === === === === === % ============================ % === === === === === === === === %
% 1031LungEQS_Tmprtr: 8.56 \times 10^(-6) xi for muscle
% 1103LungEQS_Tmprtr: low muscle xi
load('1103LungEQS_Tmprtr.mat', 'm_U', 'm_V', 'bar_d');
...
xi_2 = xi;
xi_2(3) = 2 * xi_2(3);
TargetVrtx = false(N_v, 1);
disp('Getting TargetVrtx: ');
tic;
parfor vIdx = 1: 1: N_v
...
m_U_2 = m_U;
m_V_2 = m_V;
disp('The filling time of m_U_2 and m_V_2');
tic;
parfor vIdx = 1: 1: N_v
    if TargetVrtx(vIdx)
        bioValid = false;
        ...
        for itr = 1: 1: length(CandiTet)
            ...
            if MedVal >= 3 && MedVal <= 9
                ...
                [ U_row, V_row ] = fillUVd( p1234, BndryTable, U_row, V_row, Pnt_d, ...
                            dt, Q_s_Vector(CandiTet(itr)) + Q_met(MedVal), rho(MedVal), xi_2(MedVal), zeta(MedVal), cap(MedVal), rho_b, cap_b, alpha, T_blood, T_bolus, ...
                            x_max_vertex, y_max_vertex, z_max_vertex, Vertex_Crdnt, BM_bndryNum );
            end
        end

        if bioValid
            m_U_2{vIdx} = Mrow2myRow(U_row);
            m_V_2{vIdx} = Mrow2myRow(V_row);
            % bar_d(vIdx) = Pnt_d;
        else
            m_U_2{vIdx} = [vIdx, 1];
            m_V_2{vIdx} = [vIdx, 1];
...
M_U_2   = sparse(N_v, N_v);
M_V_2   = sparse(N_v, N_v);
tic;
disp('Transfroming M_U_2 and M_V_2 from my_sparse to Matlab sparse matrix')
M_U_2 = mySparse2MatlabSparse( m_U_2, N_v, N_v, 'Row' );
M_V_2 = mySparse2MatlabSparse( m_V_2, N_v, N_v, 'Row' );
toc;
...
% the two-stage muscle xi
tic;
for idx = 2: 1: size(bar_b, 2)
    if max(bar_b(:, idx)) > 2
        [ bar_b(:, idx), flagX(idx), relres(idx) ] = gmres(M_U_2, M_V_2 * bar_b(:, idx - 1) + bar_d, int_itr_num, tol, ext_itr_num );
    else
