function sparseS_1 = PutOnDwnElctrd_Esophagus( sparseS_1, mediumTableXZ, tumor_x, tumor_y, ...
                                    dx, dy, dz, air_x, air_z, h_torso, x_max_vertex, y_max_vertex, EsBndryNum );
...
%         ell = find(mediumTableXZ(m, :) == EsBndryNum, 1, 'first');
...
%                 flag = find(arr == EsBndryNum, 1, 'first');
...
% %     % right point
% %     m_1 = find(mediumTableXZ(:, ell) == EsBndryNum, 1, 'last');
% %     x_1 = (m_1 - 1) * dx - air_x / 2;
% %     % left point
% %     m_2 = find(mediumTableXZ(:, ell) == EsBndryNum, 1, 'first');
