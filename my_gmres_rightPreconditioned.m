function [x,flag,relres,iter,resvec] = my_gmres_rightPreconditioned(A,b,restart,tol,maxit,M1,M2,x,varargin)
    ...
    for initer = 1 : inner
        ...
        if (normr <= tolb || stag >= maxstagsteps || moresteps)
            ...
            elseif evalxm == 1
                ...
                if existM1
                    additive = iterapp('mldivide',m1fun,m1type,m1fcnstr,r,varargin{:});
                    if ~all(isfinite(additive))
                        flag = 2;
                        break
                    end
                end
                if existM2
                    additive = iterapp('mldivide',m2fun,m2type,m2fcnstr,additive,varargin{:});
                    if ~all(isfinite(additive))
                        flag = 2;
                        break
                    end
