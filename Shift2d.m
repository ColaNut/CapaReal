...
% temporarily apply the shifted-grid; May needed to implemet the un-shifted version.
...
for vIdx = 1: 1: x_max_vertex * y_max_vertex * z_max_vertex
    ...
    % Artificial SheetPntsTable for regular tet in case 1
    if n_v > 6 && n_v < 20
        arti = ones(2, 3, 2);
        if m_v == 6 && ell_v == 12  
            % quadrant III:
            arti(1, :, 2) = 0;
            SheetPntsTable(m_v - 1: m_v    , n_v - 1: n_v + 1, ell_v    : ell_v + 1) = arti;
        elseif m_v == 12 && ell_v == 12
            arti(2, :, 2) = 0;
            SheetPntsTable(m_v    : m_v + 1, n_v - 1: n_v + 1, ell_v    : ell_v + 1) = arti;
        elseif m_v == 12 && ell_v == 6 
            arti(2, :, 1) = 0;
            SheetPntsTable(m_v    : m_v + 1, n_v - 1: n_v + 1, ell_v - 1: ell_v    ) = arti;
        elseif m_v == 6  && ell_v == 6 
            arti(1, :, 1) = 0;
            SheetPntsTable(m_v - 1: m_v    , n_v - 1: n_v + 1, ell_v - 1: ell_v    ) = arti;
        end
    end
end
        ...
        for itr = TetFinder + 1: length(Candi)
            if uG( Candi(TetFinder), Candi(itr) )
                ...
                [ K1_6, K2_6, Kev_4, Kve_4, B_k_Pnt ] = fillK( P1(lGidx), P2(lGidx), Candi(itr), Candi(TetFinder), ...
                    ...
                    lGidx, K1_6, K2_6, Kev_4, Kve_4, B_k_Pnt, J_0, MedVal, epsilon_r, mu_r, x_max_vertex, y_max_vertex, z_max_vertex, Vertex_Crdnt, 'Regular' );
            end
        end
    end
    if isempty(K1_6)
