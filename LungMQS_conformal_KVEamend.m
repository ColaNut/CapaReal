...
validEdge_KVE = false(l_G, 1);
% apply MedReference as a checker.
tic;
    ...
    for TetFinder = 1: 1: length(Candi) - 1
        for itr = TetFinder + 1: length(Candi)
            if uG( Candi(TetFinder), Candi(itr) )
                ...
                if counter == 1
                    MedReference = MedTetTable( tetRow, v1234(1) );
                else
                    MedVal = MedTetTable( tetRow, v1234(1) );
                    if MedVal ~= MedReference
                        validEdge(eIdx) = true;
                    end
                end
            end
...
% === % ============================== % === %
% === % Get Fat and Bone Related Edges % === %
% === % ============================== % === %
        ...
        for TetFinder = 1: 1: length(Candi) - 1
            for itr = TetFinder + 1: length(Candi)
                if uG( Candi(TetFinder), Candi(itr) )
                    ...
                    Kve_4 = fillK_FW_currentsheet_KVEamend( P1(eIdx), P2(eIdx), Candi(itr), Candi(TetFinder), ...
                        G( :, P1(eIdx) ), G( :, P2(eIdx) ), G( :, Candi(itr) ), G( :, Candi(TetFinder) ), Vrtx_bndry, J_0, ...
                        Kve_4, zeros(1, 3), MedVal, epsilon_r, mu_r, x_max_vertex, y_max_vertex, z_max_vertex, Vertex_Crdnt );
                end
            end
        end
