% % === === % ========================================================== % === === %
% % === === % Title: Lung EQS with adipose and temperature-dependendt xi % === === %
% % === === % Date : 03/03, 2018                                         % === === %
% % === === % ========================================================== % === === %
...
% % warning messages occurr in the above determination of SegMed, which are ammended by the below SegMed determination process
% for idx = 1: 1: x_idx_max * y_idx_max * z_idx_max
%     % idx = ( ell - 1 ) * x_idx_max * y_idx_max + ( n - 1 ) * x_idx_max + m;
%     [ m, n, ell ] = getMNL(idx, x_idx_max, y_idx_max, z_idx_max);
%     p0 = idx;
...
% % save('20180303_EQS_Phi_nofat.mat');
...
% % Xtable = [];
% % Ztable = [];
% % [ Xtable, Ztable ] = fillGridTable( 0, 0, muscle_a, muscle_c, dx, dz );
% % Xtable = Xtable{1};
% % Xtable = Ztable{1};
% [ Xtable, Ztable ] = fillTradlElctrd( paras(4), paras(5), dx, dz );
% [ sparseA, B, UpElecTb ] = UpElectrode( sparseA, B, Xtable, Ztable, paras, V_0, x_idx_max, y_idx_max, dx, dy, dz, z_idx_max );
% CurrentEst;
...
% % bar_x_my_gmresPhi = sqrt(2) * bar_x_my_gmresPhi;
% % === % ==================================== % === %
...
% === % ==================== % === %
load('E:\Kevin\20180811_case4_nonLinear\20180816_LungEQS_case4_nonlinear.mat');

              %     1      2       3     4       5      6       7                       8                    9
              % [ air, bolus, muscle, lung,  tumor,  bone,    fat, saline injected muscle, saline injected fat ]';
              % [ air, bolus, muscle, lung,  tumor,  bone,   fat ]';
mu_prime      = [   1,     1,      1,     1,     1,     1,     1,                       1,     1 ]';
mu_db_prime   = [   0,     0,      0,     0,     0,     0,     0,                       0,     0 ]';
% mu_db_prime   = [ 0,    0.62 ]';
...
load('E:\Kevin\CapaReal\0715\0715K1.mat', 'G', 'm_K1');
return;
starts = [];
ends = [];
vals = [];
...
MedTetTableCell_A  = cell(0, 1);
% add the following two variable: TetType and MedValTable
% TetType        = cell(0, 1);
MedValTable    = zeros(0, 1, 'uint8');
% rearrange SigmaE and Q_s; construct the MedTetTableCell
for idx = 1: 1: x_idx_max * y_idx_max * z_idx_max
    [ m, n, ell ] = getMNL(idx, x_idx_max, y_idx_max, z_idx_max);
    m_v = 2 * m - 1;
    ...
    PntJ_xyz        = zeros(48, 3);
    PntMedTetTableCell  = cell(48, 1);
    % PntTetType          = cell(48, 1, 'uint8');
    PntMedValTable  = zeros(48, 1);
    % PntQ_s          = zeros(48, 1);
    % rearrange (6, 8, 3) to (48, 3);
    tmp = zeros(8, 6);
    tmp = squeeze( SigmaE(m, n, ell, :, :, 1) )';
    ...
    tmp = squeeze( SigmaE(m, n, ell, :, :, 3) )';
    PntJ_xyz(:, 3) = tmp(:);
    tmp = squeeze( SegMed(m, n, ell, :, :) )';
    PntMedValTable = tmp(:);
    % to-do
    % tmp = squeeze(Q_s(m, n, ell, :, :))';
    % PntQ_s = tmp(:);
    % add MedTetTableCell with vertex numbering, namely [1, 3, 2, 4] for P1, P3, P2 and P4.
    % modify the getPntMedTetTable_2, accordingly.
    % modify all the PntMedTetTableCell related code.
    % PntTetType = get48TetType;
    PntMedTetTableCell = getPntMedTetTable_3( N_v, m_v, n_v, ell_v, x_max_vertex, y_max_vertex, z_max_vertex );
    validTet = find( squeeze( SegMed(m, n, ell, :, :) )' ~= byndCD );

    % set a inf and nan checker for PntTetTable_Ix, PntTetTable_Iy and PntTetTable_Iz
    % start from here: the temperature is getting lower, and the Q_s and J_xyz is in the order 0.35 and 0.002, respectively.
    J_xyz            = vertcat(J_xyz, PntJ_xyz(validTet, :));
    MedTetTableCell_A  = vertcat(MedTetTableCell_A, PntMedTetTableCell(validTet));
    MedValTable      = vertcat(MedValTable, PntMedValTable(validTet));
    % TetType          = vertcat(TetType, PntTetType(validTet));
...
if size(MedTetTableCell_A, 1) ~= validNum
    error('check the construction');
end

MedTetTable_A = sparse(validNum, N_v);
tic;
disp('Transfroming MedTetTable_A from my_sparse to Matlab sparse matrix')
MedTetTable_A = mySparse2MatlabSparse( MedTetTableCell_A, validNum, N_v, 'Row' );
toc;
    ...
    for TetFinder = 1: 1: length(Candi) - 1
        for itr = TetFinder + 1: length(Candi)
            if uG( Candi(TetFinder), Candi(itr) )
                ...
                % the following finder may cause a lot.
                tetRow = find( sum( logical(MedTetTable_A(:, v1234)), 2 ) == 4 );
                if length(tetRow) ~= 1
                    error('check te construction of MedTetTable_A');
                end
                MedVal = MedValTable( tetRow );
                Counter = [ MedTetTable_A( tetRow, P1(eIdx) ), MedTetTable_A( tetRow, P2(eIdx)), ...
                                        MedTetTable_A( tetRow, Candi(itr) ), MedTetTable_A( tetRow, Candi(TetFinder) ) ];
                ...
                [ B_k_Pnt ] = fillK1_FW_2_20180826( P1(eIdx), P2(eIdx), Candi(itr), Candi(TetFinder), Counter, ...
                    G( :, P1(eIdx) ), G( :, P2(eIdx) ), G( :, Candi(itr) ), G( :, Candi(TetFinder) ), ...
                    Vrtx_bndry( P1(eIdx) ), Vrtx_bndry( P2(eIdx) ), Vrtx_bndry( Candi(itr) ), Vrtx_bndry( Candi(TetFinder) ), ...
                    B_k_Pnt, J_xyz(tetRow, :), MedVal, epsilon_r, mu_r, x_max_vertex, y_max_vertex, z_max_vertex, Vertex_Crdnt );
            end
        end
    end
