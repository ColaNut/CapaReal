...
% % % % % for PartNum = 1: 1: 8
% % % % %     load(strcat('D:\Kevin\CapaReal\0808\0808MQS_conformal_postK', num2str(PartNum), '.mat'), 'B_k', 'm_K1');
...
% tic;
% disp('The gmres solutin of Ax = B: ');
% bar_x_my_gmres = gmres( nrmlM_K, nrmlB_k2, int_itr_num, tol, ext_itr_num, L_K, U_K );
% toc;
% % tic;
% % disp('The gmres solutin of Ax = B: ');
    ...
    for itr = 1: 1: length(CandiTet)
        ...
        if MedVal >= 3 && MedVal <= 9
            ...
            Pnt_d = filld( p1234, Vrtx_bndry, Pnt_d, ...
                            dt, Q_s_Vector(CandiTet(itr)) + Q_met(MedVal) + Q_s_MNP(CandiTet(itr)) / 80^2, rho(MedVal), xi(MedVal), zeta(MedVal), cap(MedVal), rho_b, cap_b, alpha, T_blood, T_bolus, ...
                            x_max_vertex, y_max_vertex, z_max_vertex, Vertex_Crdnt, BM_bndryNum );
        end
    end
