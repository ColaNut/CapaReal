...
% % % epsilon_r_pre = [   1,    80,   9658,  7175,  8952,   76.99, 50.04 ]';
...
% % % tic;
% % % disp('Getting validIndices: ');
% % % validIndices = zeros(x_idx_max * y_idx_max * z_idx_max, 1);
% % % % rearrange SigmaE and Q_s; construct the MedTetTableCell
% % % for idx = 1: 1: x_idx_max * y_idx_max * z_idx_max
% % %     [ m, n, ell ] = getMNL(idx, x_idx_max, y_idx_max, z_idx_max);
% % %     validTet = find( squeeze( SegMed(m, n, ell, :, :) )' ~= byndCD );
% % %     validIndices(idx) = length(validTet);
% % % end
% % % toc;
% % % cumsumTet = zeros(size(validIndices));
% % % cumsumTet = cumsum(validIndices);
% % % tic;
...
% % %         MedTetTableCell( cumsumTet(idx - 1) + 1: cumsumTet(idx) )  = PntMedTetTableCell(validTet);   
% % %     end
% % % end
% % % toc;
...
% % load('0808MQS_conformal_preK.mat');
...
% % % get bone and fat related edges
% % validEdge = false(l_G, 1);
% % % tic;
% % % disp('Get valid edges');
% % % parfor eIdx = 1: 1: l_G
% % %     Candi = [];
...
% % load('0811ValidEdges.mat');

% % length(find(validEdge))

% %               % [ air,   air, muscle,  lung, tumor,    bone,   fat ]';
% % rho           = [   1,  1020,   1020,   394,   697,   1020,   1020 ]';
% % epsilon_r_pre = [   1,     1,   9658,  7175,  8952,   9658, 9658 ]';
% % sigma         = [   0,     0,    0.4, 0.236, 0.421,    0.4,  0.4 ]';
% % epsilon_r     = epsilon_r_pre - i * sigma ./ ( Omega_0 * Epsilon_0 );
...
% % firstIdx = 857089;
% % diffIdx = l_G - firstIdx;

% % switch PartNum
% %     case 1
% %         Rng = [ firstIdx, ceil(firstIdx + diffIdx / 4) ];
% %     case 2
% %         Rng = [ ceil(firstIdx + diffIdx / 4) + 1, ceil(firstIdx + diffIdx / 2) ];
% %     case 3
% %         Rng = [ ceil(firstIdx + diffIdx / 2) + 1, ceil(firstIdx + 3 * diffIdx / 4) ];
% %     case 4
% %         Rng = [ ceil(firstIdx + 3 * diffIdx / 4) + 1, l_G ];
% %     otherwise
% %         error('check');
% % end
...
% % tic; 
% % disp( strcat('The filling time of K_1, K_EV, K_VE and B: part', num2str(PartNum)) );
% % parfor eIdx = Rng(1): 1: Rng(2)
% %     % eIdx = full( G(P2(lGidx), P1(lGidx)) );
% %     if validEdge(eIdx)
% %     % if TumorRelatedV( P1(eIdx) ) || TumorRelatedV( P2(eIdx) )
% %         Candi = [];
% %         % get candidate points
% %         P1_cand = uG(:, P1(eIdx));
% %         P2_cand = uG(:, P2(eIdx));
...
% load('D:\Kevin\CapaReal\0721\0721K_Q1.mat', 'm_K1');
% % load('D:\Kevin\CapaReal\0721\0721K_Q1.mat', 'm_K1', 'm_K2', 'm_KEV', 'm_KVE');
...
% load('D:\Kevin\CapaReal\0721\0721K_Q2.mat', 'm_K1');
% % load('D:\Kevin\CapaReal\0721\0721K_Q2.mat', 'm_K1', 'm_K2', 'm_KEV', 'm_KVE');
...
% load('D:\Kevin\CapaReal\0721\0721K_Q3.mat', 'm_K1');
% % load('D:\Kevin\CapaReal\0721\0721K_Q3.mat', 'm_K1', 'm_K2', 'm_KEV', 'm_KVE');
...
% load('D:\Kevin\CapaReal\0721\0721K_Q4.mat', 'm_K1');
% % load('D:\Kevin\CapaReal\0721\0721K_Q4.mat', 'm_K1', 'm_K2', 'm_KEV', 'm_KVE');
...
% % === === % ================================== % === === %
% % === === % Amend for mu'' in tumor region ??? % === === %
% % === === % ================================== % === === %

% for PartNum = 1: 1: 8
%     load(strcat('E:\Kevin\CapaReal\0808\0808MQS_conformal_postK', num2str(PartNum), '.mat'), 'B_k', 'm_K1');
...
% % for PartNum = 1: 1: 4
% %     load( strcat('D:\Kevin\CapaReal\0808\0808MQS_conformal_postK', num2str(PartNum), '_noFatAndBone.mat' ), 'm_K1', 'm_K2', 'm_KEV', 'm_KVE');
% %     % ne_B_k = find(~cellfun(@isempty,B_k));
% %     ne_m_K1 = find(~cellfun(@isempty,m_K1));
% %     ne_m_K2 = find(~cellfun(@isempty,m_K2));
% %     ne_m_KEV = find(~cellfun(@isempty,m_KEV));
...
% % % save('0808LungConformal_preGVVinv.mat', 'sparseGVV', 'N_v');
...
% % load('0808preProdct.mat', 'M_K1', 'N_e', 'B_k', 'Mu_0', 'Omega_0', 'M_K2', 'M_KEV', 'M_sparseGVV_inv_spai', 'M_KVE');
...
% parameters for 8 MHz 
Omega_0       = 2 * pi * 1.2 * 10^6; % 2 * pi * 1.2 MHz
              % [ air, bolus, muscle, lung,  tumor,    bone,    fat ]';
epsilon_r_pre = [   1,    80,   1387,  1102,  1589,    22.6,  20.31 ]';
sigma         = [   0,     0,   0.54,  0.33,  0.53, 0.00283, 0.0237 ]';
epsilon_r     = epsilon_r_pre - i * sigma ./ ( Omega_0 * Epsilon_0 );
...
% Q_s_MNP_mod = Q_s_MNP;
Q_s_MNP_mod = Q_s_MNP * ( 400 / 5000 )^2;
% Q_s_Vector_mod = Q_s_Vector;
Q_s_Vector_mod = Q_s_Vector * ( 400 / 5000 )^2;
...
% a modification of J = 5,000 to J = 212.5 should be incorporated