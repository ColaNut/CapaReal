...
load('D:\Kevin\CapaReal\0715\Power300.mat', 'Q_s_Vector');
Q_s_Vector_amend = 2 * Q_s_Vector;
bar_d = zeros(N_v, 1);
    ...
    for itr = 1: 1: length(CandiTet)
        ...
        if MedVal >= 3 && MedVal <= 9
            ...
            Pnt_d = filld( p1234, Bls_bndry, Pnt_d, ...
                            dt, Q_s_Vector_amend(CandiTet(itr)) + Q_met(MedVal), rho(MedVal), xi(MedVal), zeta(MedVal), cap(MedVal), rho_b, cap_b, alpha, T_blood, T_bolus, ...
...
save('Pnt_d_PowerTwofold.mat', 'Pnt_d');
