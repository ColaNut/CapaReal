...
if T_flagXZ == 1
    figure(21);
    clf;
    tumor_n_B   = ( tumor_y_es - 1.5 / 100 - 0 ) / dy_B + ( w_y_B + dy ) / (2 * dy_B) + 1;
    ...
    saveas(figure(21), 'EsoMQSTmprtrXZ1017_short.jpg');
end

if T_flagXY == 1
    disp('Checking bio and bolus related vetrices XY: ');
    tic;
    ...
    % plotGridLineXZ( shiftedCoordinateXYZ, uint64(y / dy + h_torso / (2 * dy) + 1) );
    saveas(figure(22), 'EsoMQSTmprtrXY1017_short.jpg');
end
...
if T_flagYZ == 1
    ...
    % plotMap( paras2dXZ, dx, dz, top_x0, top_dx, down_dx );
    % plotGridLineXZ( shiftedCoordinateXYZ, uint64(y / dy + h_torso / (2 * dy) + 1) );
    saveas(figure(23), 'EsoMQSTmprtrYZ1017_short.jpg');
end
